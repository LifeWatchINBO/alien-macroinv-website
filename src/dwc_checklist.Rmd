...
# Generate taxonID:
raw_data %<>% mutate(taxon_id = paste("alien-macroinvertebrates-checklist", "taxon", vdigest(scientific_name, algo="md5"), sep = ":"))
```
...
```{r}
raw_data %<>% mutate(reference = recode(source,
...
```{r}
taxon %<>% mutate(scientificName = raw_scientific_name) %>% mutate(scientificName = str_trim(scientificName))
```
...
taxon %<>% mutate(taxonRank = case_when(
  raw_scientific_name == "Dreissena rostriformis bugensis" ~ "subspecies",
  raw_scientific_name != "Dreissena rostriformis bugensis" ~ "species")
)
```
...
The date information can be found in `raw_date_first_observation`, but it is currently expressed in different formats: `yyyy`, `< yyyy`, `<yyyy`, `before yyyy` and `yyyy-yyyy`:

```{r}
distribution %>%
  distinct(raw_date_first_observation) %>%
  arrange(raw_date_first_observation)
```

We remove the special characters from `raw_date_first_observation` and save as `year`:

```{r}
distribution %<>% mutate(year = str_replace_all(raw_date_first_observation, "(< |before |<)", ""))
```
...
distribution %<>%
  separate(
    year,
    into = c("date_first_observation", "date_last_observation"),
    sep = "-") %>% 
  mutate(
    eventDate = case_when(
...
Compare formatted dates with original dates in `raw_date_first_observation`:

```{r}
distribution %>%
  distinct(raw_date_first_observation, eventDate) %>%
  arrange(raw_date_first_observation)
```
...
```{r}
distribution %<>% select(-starts_with("raw_"), -date_first_observation, -date_last_observation)
```
